...
export async function createAppointment(fd:FormData){
  ...
  const ids=[...new Set([val(fd.get("studentId")),...fd.getAll("partnerId").map(val)].filter(Boolean))];
