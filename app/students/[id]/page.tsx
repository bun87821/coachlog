...
export default async function StudentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="shell">
    ...
    <aside className="student-quick-nav" aria-label="學生頁快速導覽"><strong>快速前往</strong><nav><a href="#student-overview">學生資料</a><a href="#progress">進步趨勢</a><a href="#new-workout">新增訓練</a><a href="#body-metrics">身體數據</a><a href="#history">歷史紀錄</a></nav></aside>
    ...
    <section className="history-section" id="history"><div className="section-title"><div><div className="eyebrow">課程歷史</div><h2>過去訓練紀錄</h2></div><span className="muted">共 {sessions.length} 堂</span></div>
      {sessions.length ? <div className="session-history">{sessions.map((session: any, index: number) => <article className="card session-card" key={session.id}><div className="session-date"><div>{index === 0 && <span className="latest-badge">最新紀錄</span>}<strong>{new Date(session.occurredAt).toLocaleDateString("zh-TW", { year: "numeric", month: "long", day: "numeric" })}</strong></div><time>{new Date(session.occurredAt).toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })} · {session.exercises.length} 個動作 · {session.exercises.reduce((total: number, exercise: any) => total + exercise.sets.length, 0)} 組</time></div>{session.notes && <p className="session-note">{session.notes}</p>}<div className="history-exercises">{session.exercises.map((exercise: any) => <div className="history-exercise" key={exercise.name}><h3>{exercise.name}</h3><div className="history-sets">{exercise.sets.map((set: any) => <span key={set.setNumber}>第 {set.setNumber} 組　<strong>{set.weight ?? "—"} {set.unit}</strong> × {set.reps ?? "—"}</span>)}</div></div>)}</div></article>)}</div> : <div className="card empty">尚無訓練紀錄，完成上方表單後會顯示在這裡。</div>}
    </section>
  </main>;
}
