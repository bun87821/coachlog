...
export function LocalCoachApp() {
  ...
  if (student) return <LocalStudentPage data={data} studentId={student.id} partnerIds={partnerIds.filter(id => id !== student.id && data.students.some(item => item.id === id))} onPartnersChange={setPartnerIds} update={update} back={() => { setPartnerIds([]); setSelected(null); }} />;
