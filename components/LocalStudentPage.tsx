...
export function LocalStudentPage({ data, studentId, update, back }: { data: LocalCoachData; studentId: string; update: (data: LocalCoachData) => void; back: () => void }) {
  ...
  return <main className="shell local-app">
    ...
    <section className="progress-section" id="progress"><div className="section-title"><div><div className="eyebrow">進步趨勢</div><h2>數據曲線</h2></div></div><ProgressCharts metrics={student.metrics.map(metric => ({ date: metric.date, weight: Number(metric.weight) || null, bodyFat: Number(metric.bodyFat) || null, muscle: Number(metric.muscle) || null }))} loads={loads} /></section>
    <div className="record-layout">
      ...
      <div className="card" id="body-metrics"><h2>新增身體數據</h2><form className="stack" onSubmit={event => { event.preventDefault(); const form = new FormData(event.currentTarget); replace({ ...student, metrics: [...student.metrics, { id: uid(), date: String(form.get("date")), weight: String(form.get("weight") || ""), bodyFat: String(form.get("bodyFat") || ""), muscle: String(form.get("muscle") || "") }] }); }}><label>測量日期<input name="date" type="date" defaultValue={today()} required /></label><div className="row"><label>體重 kg<input name="weight" type="number" step=".1" /></label><label>體脂 %<input name="bodyFat" type="number" step=".1" /></label></div><label>肌肉量 kg<input name="muscle" type="number" step=".1" /></label><button>儲存身體數據</button></form></div>
    </div>
