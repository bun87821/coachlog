...
export function ProgressCharts({ metrics, loads }: { metrics: any[]; loads: any[] }) {
  ...
  return <div className="grid">
    <div className="card body-chart" style={{ gridColumn: "span 2" }}>
      <h3>身體組成趨勢</h3>
      {metrics.length ? <div className="chart"><ResponsiveContainer><LineChart data={metrics}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="date" /><YAxis /><Tooltip /><Legend /><Line type="monotone" dataKey="weight" name="體重 kg" stroke="#176b45" /><Line type="monotone" dataKey="bodyFat" name="體脂 %" stroke="#e47c45" /><Line type="monotone" dataKey="muscle" name="肌肉量 kg" stroke="#425fc7" /></LineChart></ResponsiveContainer></div> : <div className="chart-empty">輸入兩次以上身體數據後，這裡會顯示變化曲線。</div>}
    </div>
    <div className="card">
