...
export function BookingCalendar({ students, events, settings, blocks, initialDate }: { students: Student[]; events: CalendarEvent[]; settings: Settings; blocks: AvailabilityBlock[]; initialDate?: string }) {
  ...
  return <section className="calendar-v2">
    ...
    <div className="card appointment-card" id="appointment-form"><h2>新增學生預約</h2>{students.length ? <form className="stack" action={createAppointment}><label>學生<select name="studentId" required value={mainStudent} onChange={event => { setMainStudent(event.target.value); setPartnerIds(ids => ids.filter(id => id !== event.target.value)); }}><option value="" disabled>選擇學生</option>{students.map(student => <option value={student.id} key={student.id}>{student.name}</option>)}</select></label>{students.length > 1 && <fieldset className="booking-partners"><legend>一起上課的學生（選填）</legend><div className="participant-list">{students.filter(student => student.id !== mainStudent).map(student => { const included = partnerIds.includes(student.id); return <label className={`participant-chip ${included ? "included" : ""}`} key={student.id}><input type="checkbox" name="partnerId" value={student.id} checked={included} onChange={() => setPartnerIds(ids => included ? ids.filter(id => id !== student.id) : [...ids, student.id])} />{student.name}</label>; })}</div></fieldset>}<label>日期<input name="date" type="date" value={selectedDate} onChange={event => setSelectedDate(event.target.value)} required /></label><div className="row stacked-fields"><label>開始時間<input name="time" type="time" defaultValue={availableSlots[0] || "10:00"} required /></label><label>課程分鐘<input name="duration" type="number" min="15" step="15" defaultValue={settings.duration} required /></label></div><textarea name="notes" placeholder="預約備註（選填）" /><button>加入 Google Calendar</button></form> : <p className="muted">請先建立學生，再新增預約。</p>}</div>
