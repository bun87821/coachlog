...
export function BookingCalendar({ students, events, settings, blocks, initialDate }: { students: Student[]; events: CalendarEvent[]; settings: Settings; blocks: AvailabilityBlock[]; initialDate?: string }) {
  ...
  return <section className="calendar-v2">
    ...
    <div className="day-schedule">
      <section><h3>已預約</h3>{selectedEvents.length ? selectedEvents.map(event => { const studentIds = [...(event.description?.matchAll(/student:([0-9a-f-]+)/g) || [])].map(match => match[1]); const studentId = studentIds[0]; return <a className="schedule-row booked" key={event.id} href={studentId ? `/students/${studentId}${studentIds.length > 1 ? `?with=${studentIds.slice(1).join(",")}` : ""}` : event.htmlLink} target={studentId ? undefined : "_blank"}><time>{timeFor(event.start?.dateTime)}</time><strong>{event.summary || "未命名預約"}</strong><span>{event.end?.dateTime ? `${timeFor(event.end.dateTime)} 結束` : "全天"}</span></a>; }) : <p className="muted">這天目前沒有預約。</p>}</section>
